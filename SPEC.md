...
- **Primer club con contraseña de 3 símbolos — dos métodos a elección.** Al crear tu PIN eliges: **3 emojis** (set serio/synthwave: ⚡🌙🔥💀🗲☄️🌑🩸🖤⚜️♾️🔱🧿🌌🛸🔺⛓️🌀, nada infantil, pensado para 28-52 años) **o 3 dígitos numéricos** (0-9), lo que te sea más fácil de recordar. Debes volver a entrar con el mismo método que elegiste — no son intercambiables entre sí, son dos alternativas para crear tu llave.
- Flujo: tocas tu personaje → si nadie lo ha tomado, eliges método (emoji o dígitos) y 3 símbolos en orden, quedas registrado (`olimpo_personajes/{nombre}` en Firestore, guardando el hash SHA-256 del PIN, nunca el PIN en claro); si ya está tomado, debes ingresar el mismo PIN para "ser" ese personaje — así nadie suplanta a nadie.
...
- **Recuperación de acceso:** si alguien olvida su PIN (sea emoji o dígitos), no hay self-service — debe pedirle a Jose que libere su nombre borrando el documento `olimpo_personajes/{nombre}` manualmente desde la consola de Firebase, y vuelve a registrarse desde cero. Reset vía email (Cloud Functions) queda para v1.2.
