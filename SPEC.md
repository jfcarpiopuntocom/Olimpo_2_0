...
- **Avatares cyborg reales, provistos por JFC** (`avatars/cy1.png`...`cy8.png`, generados con Grok, fondo transparente). Reemplazaron un primer intento de SVG dibujado a mano (descartado por floja calidad visual). Solo **8 diseños únicos** existen (las 3 imágenes que mandó JFC eran el mismo set de 8 en 3 estilos de color, no 24 distintos), y de esos 8 **solo 2 leen como mujer** (`cy4`, `cy5`). Mapeo a los 11 fundadores en `FOUNDER_AVATAR` (`index.html`) respetando género donde el nombre lo indica — Anahi/Laura/Maria comparten esos 2 diseños femeninos (Maria repite el de Anahi, no hay un tercero distinto disponible). Nicknames nuevos ciclan por hash entre los 8, sin garantía de "género correcto" porque no hay forma de saberlo de un nickname libre. **Pendiente:** si se quiere un ratio más parejo (JFC pidió 4+ mujer / 5+ hombre), hace falta generar 2-3 diseños mujer más en el mismo estilo — documentado, no resuelto, JFC no respondió qué camino prefiere (generar más vs. SVG propio de relleno vs. usar lo que hay).
